...
open class Matrix<T : Any> {
    ...
    constructor(size: Size, init: (i: Int, j: Int) -> Any) {
        this.matrix = Array(size.width) { i -> Array(size.height) { j -> init(i, j) } }
        this.size = size
    }
...
//        val s=calculataBufferSize(wStart, hStart, size)
    ...
    fun splitBuffered(horizontalStep: Int, verticalStep: Int): Matrix<Matrix<T>> {
        ...
        val res1 = Matrix<Matrix<T>>(s.width, s.height) { i, j ->
            rectBufferSave(
                i * horizontalStep,
                j * verticalStep,
                i * horizontalStep + horizontalStep,
                j * verticalStep + verticalStep
            )
        }
        return res1
    }
    ...
    fun split(horizontalStep: Int, verticalStep: Int): Matrix<Matrix<T>> {
        val s = calculateMatrixOfMatrixSize(horizontalStep, verticalStep)
        val res1 = Matrix<Matrix<T>>(s) { i, j ->
            rectSaveIterator(
                i * horizontalStep,
                j * verticalStep,
                Size(horizontalStep, verticalStep)
            )
        }
        return res1
    }
    ...
    companion object {
        @JvmStatic
        fun <T : Any> valueOf(mat: Array<Array<T>>): Matrix<T> {
            return Matrix<T>(mat as Array<Array<Any>>)
        }

        @JvmStatic
        fun <T : Any> glueTogether(mat: Matrix<Matrix<T>>): Matrix<T> {
            return Matrix<T>(mat as Array<Array<Any>>)
        }
    }
}